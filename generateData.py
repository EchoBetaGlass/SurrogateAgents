...
for problem in problems:
    for num_var in num_vars:
        ...
        for num_sample in num_samples:
            ...
            data_uniform.to_csv(filename + "uniform.csv")
            ...
            data_noisy.to_csv(filename + "uniform_noisy.csv")
            var_norm = var
            for i in range(2):
            ...
            data_normalized.to_csv(filename + "normal.csv")
            for i in range(num_var + num_obj):
                data_noisy[:, i] = data_noisy[:, i] + np.random.normal(
                    noise_mean, noise_std, num_sample
                )
            ...
            data_noisy.to_csv(filename + "normal_noisy.csv")
